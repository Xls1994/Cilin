...
def loadCilin():
    ...
    if os.path.exists(macro.CILIN_PKL_PATH):
        return pickle.load(open(macro.CILIN_PKL_PATH))
    cs = CilinSimilarity()
    pickle.dump(cs, open(macro.CILIN_PKL_PATH, "w"))
    return cs
...
if __name__ == '__main__':
    ...
    print '-------------------------'
    print len(score_f)
    print eval.spearman(score_f, result1_f)[0]
    print eval.spearman(score_f, result2_f)[0]
    print eval.spearman(score_f, result3_f)[0]
